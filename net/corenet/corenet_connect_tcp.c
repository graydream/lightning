...
static int __corenet_tcp_get_numaid(uint32_t addr, int *numaid)
{
        int ret;
...
static int __corenet_tcp_getaddr(const core_t *core, uint32_t port,
                                 corenet_addr_t *addr, int force)
{
        ...
        for (int i = 0; i < info->info_count; i++) {
                if (force) {
                        ...
                        ret = __corenet_tcp_get_numaid(addr->info[i].addr, &numaid);
                        if (ret)
                                continue;
...
int corenet_tcp_getaddr(uint32_t port, corenet_addr_t *addr)
{
        ...
        ret = __corenet_tcp_getaddr(core, port, addr, 0);
        if (ret) {
                if (ret == ENODEV) {
                        ret = __corenet_tcp_getaddr(core, port, addr, 1);
                        if (ret)
                                GOTO(err_ret, ret);
                } else {
                        GOTO(err_ret, ret);
