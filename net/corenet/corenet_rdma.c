...
#define CMID_DUMP(cmid) DINFO("cm_id %p verbs %p pd %p\n", \
        (cmid), \
        (cmid)->verbs, \
        (cmid)->pd);
...
retry:
        ret = core_request(core->hash, -1, "rdma_timewait",
                           __corenet_rdma_tw_exit, rdma_handler);
        if (ret) {
                if (ret == ENOSPC) {
                        DWARN("core request queue is full, sleep 5ms will retry\n");
                        usleep(5000);
