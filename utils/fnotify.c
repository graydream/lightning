...
static void *__fnotify_thr_fn(void *arg)
{
        ...
        while (srv_running) {
                ...
                for (idx = 0; idx < len; idx += sizeof(struct inotify_event) + event->len) {
                        ...
                        ret = ltg_spin_lock(&fnotify.lock);
                        if (unlikely(ret))
                                GOTO(err_ret, ret);
...
int fnotify_init()
{
        ...
        ret = ltg_spin_init(&fnotify.lock);
        if (unlikely(ret))
                GOTO(err_ret, ret);
                     ...
                     fnotify_callback del_callback, void *context)
{
        ...
        ret = ltg_malloc((void **)&ent, sizeof(*ent) + strlen(path) + 1);
        if (unlikely(ret))
                GOTO(err_ret, ret);
        ...
        //strncpy(ent->buf, buf, sizeof(ent->buf)  1);
