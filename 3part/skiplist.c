...
#if 0
err_head:
        ltg_free((void **)&list->head);
#endif
err_nil:
        ltg_free((void **)&list->nil);
err_list:
        ltg_free((void **)&list);
err_ret:
        return ret;
...
int skiplist_destroy(struct skiplist *slist)
{
        if (slist != NULL) {
                ...
                if (slist->btlist != NULL)
                        ltg_free((void **)&slist->btlist);
#endif
                if (slist->head != NULL)
                        ltg_free((void **)&slist->head);
                if (slist->nil != NULL)
                        ltg_free((void **)&slist->nil);
                ltg_free((void **)&slist);
        }
...
int skiplist_clear(struct skiplist *slist, int free)
{
        ...
        LTG_ASSERT(skiplist_get_size(slist) == 0);
