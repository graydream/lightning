 ...
 * lowest-order bit. The X^32 term is "implied"; the LSB is the X^31 term, etc.
 * The X^0 term (usually shown as "+1") results in the SMB being 1
 *
 * Note that the usual hardware shift register implementation, which is what
 * we're using (we're merely optimizing it by doing eight-bit chunks at a time)
 ...
 * term. UARTs transmit characters in order from LSB to MSB. By storing the
 * CRC this way we hand it to the UART in the order low-byte to high-byte; the
 * UART sends each low-bit to hight-bit; and the result is transmission bit by
...
void crc32_md(void *ptr, uint32_t len)
{
        ...
        crc32_stream(&crcode, crc->buf, len - sizeof(crc_t));

        crc32_stream_finish(crcode);
...
uint32_t crc32_sum(const void *ptr, uint32_t len)
{
        ...
        crc32_stream(&crcode, ptr, len);

        return crc32_stream_finish(crcode);
}
